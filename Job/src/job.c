...
void obtenerConfiguracion(char* path) {
	...
	configuracion = (t_Datos_configuracion*) malloc(
			sizeof(t_Datos_configuracion));
	config = config_create(path);

	if (config_has_property(config, "IP_MARTA")) {
		configuracion->ip_marta = strdup(
				config_get_string_value(config, "IP_MARTA"));
	} else {
		log_error_consola(
				"El archivo de configuración %s debe tener un IP_MARTA", path);
		exit(1);
	}
	if (config_has_property(config, "REDUCE")) {
		configuracion->reduce = read_whole_file(
				config_get_string_value(config, "REDUCE"));
	} else {
		log_error_consola("El archivo de configuración %s debe tener un REDUCE",
				path);
		exit(1);
	}
	if (config_has_property(config, "MAPPER")) {
		configuracion->mapper = read_whole_file(
				config_get_string_value(config, "MAPPER"));
	} else {
		log_error_consola("El archivo de configuración %s debe tener un MAPPER",
				path);
		exit(1);
	}
	if (config_has_property(config, "PUERTO_MARTA")) {
		configuracion->puerto_marta = config_get_int_value(config,
				"PUERTO_MARTA");
	} else {
		log_error_consola(
				"El archivo de configuración %s debe tener un PUERTO_MARTA",
				path);
		exit(1);
	}
	if (config_has_property(config, "COMBINER")) {
		...
		log_error_consola(
				"El archivo de configuración %s debe tener un COMBINER", path);
		exit(1);
	}
	if (config_has_property(config, "RESULTADO")) {
		configuracion->resultado = strdup(
				config_get_string_value(config, "RESULTADO"));
	} else {
		log_error_consola(
				"El archivo de configuración %s debe tener un RESULTADO", path);
		exit(1);
	}
	if (config_has_property(config, "ARCHIVOS")) {
		configuracion->archivos = strdup(
				config_get_string_value(config, "ARCHIVOS"));
	} else {
		log_error_consola(
				"El archivo de configuración %s debe tener un ARCHIVOS", path);
		exit(1);
	}

	log_info_consola("El archivo de configuración %s fue cargado con éxito",
			path);
...
void conectarseAMarta() {
	marta_sock = client_socket(configuracion->ip_marta,
			configuracion->puerto_marta);

	if (marta_sock < 0) {
		log_error_consola("No se pudo conectar a MaRTA IP: %s - Puerto: %d",
				configuracion->ip_marta, configuracion->puerto_marta);
		exit(1);
	}

	log_debug_consola("Se conectó al proceso MaRTA IP: %s - Puerto: %d",
			configuracion->ip_marta, configuracion->puerto_marta);
...
int hiloReduce(void* dato) {
	...
	if (nodo_sock < 0) {
		log_error_consola(
				"No se pudo conectar al proceso %s - IP: %s - Puerto: %d",
				args->nombre_nodo, args->ip, args->puerto);
		mensaje_respuesta = argv_message(FIN_REDUCE_ERROR, 1,
				args->id_operacion);
	} else {

		log_debug_consola("Se conectó al proceso %s - IP: %s - Puerto: %d",
				args->nombre_nodo, args->ip, args->puerto);
		...
		log_debug_interno(
				"Enviando mensaje de solicitud de reduce. Header.ID: %s - Argc: %d - Largo Stream: %d",
				id_string(mensaje->header.id), mensaje->header.argc,
		...
		log_debug_interno(
				"Enviando mensaje rutina. Header.ID: %s - Argc: %d - Largo Stream: %d",
				id_string(mensaje->header.id), mensaje->header.argc,
				mensaje->header.length);
		...
		for (i = 0; i < args->archivos_tmp->elements->elements_count; i++) {
			...
			log_debug_interno(
					"Enviando mensaje archivos de reduce. Header.ID: %s - Argc: %d - Largo Stream: %d",
					id_string(mensaje->header.id), mensaje->header.argc,
					mensaje->header.length);
			ret = enviar_mensaje(nodo_sock, mensaje);
		}
		...
		log_debug_interno(
				"Enviando mensaje fin de Mensaje. Header.ID: %s - Argc: %d - Largo Stream: %d",
				id_string(mensaje->header.id), mensaje->header.argc,
				mensaje->header.length);
		...
		if (!mensaje) { //Significa que recibir_mensaje devolvió NULL o sea que hubo un error en el recv o el nodo se desconectó
			...
			mensaje_respuesta = argv_message(mensaje->header.id, 1,
					args->id_operacion);
			log_debug_interno(
					"Se recibió mensaje de %s. Header.Id: %s - Argc: %d - Largo Stream: %d",
					args->nombre_nodo, id_string(mensaje->header.id),
	...
	log_debug_interno(
			"Enviando mensaje respuesta a MaRTA. Header.ID: %s - Argc: %d - Largo Stream: %d",
			id_string(mensaje_respuesta->header.id),
...
int hiloMap(void* dato) {
	...
	if (nodo_sock < 0) {
		...
		mensaje = string_message(EJECUTAR_MAP, args->archivo_final, 1,
				args->bloque);
		...
		mensaje = string_message(RUTINA, configuracion->mapper, 1,
				args->id_operacion);

		log_debug_interno(
				"Enviando mensaje de rutina. Header.ID: %s - Argc: %d - Largo Stream: %d",
				id_string(mensaje->header.id), mensaje->header.argc,
				mensaje->header.length);
		...
		if (!mensaje) { //Significa que recibir_mensaje devolvió NULL o sea que hubo un error en el recv o el nodo se desconectó
			mensaje_respuesta = argv_message(FIN_MAP_ERROR, 1,
					args->id_operacion);
		} else {
			mensaje_respuesta = argv_message(mensaje->header.id, 1,
					args->id_operacion);
...
void handshakeMarta() {
	...
	t_msg* mensaje = string_message(CONEXION_JOB, str_mensaje, 1,
			configuracion->combiner);

	log_debug_interno(
			"Enviando mensaje de solicitud de inicio de Job a MaRTA. Header.ID: %s - Archivos: %s - Resultado: %s",
			id_string(CONEXION_JOB), configuracion->archivos,
	...
	if (ret < 0) {
		log_error_consola(
				"Falló el envío de mensaje de solicitud de inicio de Job");
		exit(1);
	}
...
void levantarHiloReduce(t_params_hiloReduce* nodo) {
	...
	int dato = pthread_create(&thHiloReduce, NULL, (void *) hiloReduce,
			(void*) nodo);
	if (dato != 0) {
